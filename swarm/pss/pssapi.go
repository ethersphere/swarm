...
func (pssapi *API) SendSym(key string, topic whisper.TopicType, msg []byte) error {
	return pssapi.Pss.SendSym(key, topic, msg)
}
...
// The addresslength parameter decides how many bytes of the address to reveal in transit. -1 equals showing all. 0 means show none.
func (pssapi *API) Handshake(pubkey string, topic whisper.TopicType, to PssAddress) (string, error) {
	return pssapi.sendKey(common.FromHex(pubkey), &topic, "", to)
}
...
func (self *APITest) GetSymKey(keyid string) ([]byte, error) {
