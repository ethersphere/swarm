...
func TestSuggestPeerWTF(t *testing.T) {
	for _, v := range []struct {
		...
		t.Run(v.name, func(t *testing.T) {
			...
			err := testSuggestPeerWTF(k, v.expAddr, v.expPo, v.expChanged)
			if err != nil {
...
func testSuggestPeerWTF(k *Kademlia, expAddr string, expPo int, expChanged bool) error {
	addr, o, changed := k.SuggestPeer()
	log.Trace("suggestpeer return", "a", addr, "o", o, "want", changed)
	if binStr(addr) != expAddr {
		return fmt.Errorf("incorrect peer address suggested. expected %v, got %v", expAddr, binStr(addr))
	}
	if o != expPo {
	...
	if changed != expChanged {
		return fmt.Errorf("expected SuggestPeer to want peers: %v", expChanged)
	}
	return nil
}
