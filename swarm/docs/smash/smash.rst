...
3. Now calculate the modified version of the data. Take the :math:`j^{th}` segment of the chunk and replace it with a modified segment that is the original segment hashed with the seed appended:
...
in exactly :math:`{\depth}` steps. This essentially means that number of steps in the proof of correctness is logarithmic in chunk length.

let us examine this merkle proof in detail. We begin by introducing notation for the successive nodes of the merkle proof for the :math:`j^{th}` segment of a chunk (see also :numref:`figure %s <fig:ashproof>`):
...
Since :math:`I_C(\level, j) \mod 2` is the :math:`\level^{th}`-th least significant bit in the binary representation of :math:`j`, the index's bits inform us which order :math:`\CH` and :math:`\PH` are concatenated to yield the hash of the next level. Define the directional hash function:
...
Now we turn to the formal definition of the masked secret audit hash tree relevant for repeatable audits without remembering secrets where we outsource the storage of multiple masked secrets to the storer, necessitating storer-side merkle proofs of the masked secrets validity.

The basic idea is to store all the masked secrets in a merkle tree (MASH tree) and to remember only the root of this tree (MASH root). A successful response to a challenge contains a not just a secret, but also a merkle proof from the secret to the MASH root.
...
3. All of these masked secrets needs to be stored by storers in order to prove either the correctness of their secret or incorrectness of some seed. 
...
if the hash of the secret matches the mask in the :math:`i^{th}` position in the MASH tree, answering a challenge consists of the MASH proof of the :math:`i^{th}` mask. This is the positive response assuring the integrity of storage of the chunk. Hence the motto: SMASH-proof = *Secured by Masked Audit Secret Hash* proof. We can say the chunk is *smash-proof*.
...
This proof is also used in conjunction with the MASH proof to prove to a third part that a challenge was invalid.
