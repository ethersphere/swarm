...
func (s *GlobalStore) Get(addr common.Address, key []byte) (data []byte, err error) {
	has, err := s.db.Has(dbIndexForHashesPerNode(addr, key), nil)
	if err != nil {
...
func (s *GlobalStore) Put(addr common.Address, key []byte, data []byte) error {
	...
	batch.Put(dbIndexForHashesPerNode(addr, key), nil)
	batch.Put(dbIndexForNodesWithHash(key, addr), nil)
	batch.Put(dbIndexForNodes(addr), nil)
	batch.Put(dbIndexForHashes(key), nil)
...
func (s *GlobalStore) Delete(addr common.Address, key []byte) error {
	...
	batch := new(leveldb.Batch)
	batch.Delete(dbIndexForHashesPerNode(addr, key))
	batch.Delete(dbIndexForNodesWithHash(key, addr))

	// check if this node contains any keys, and if not
	// remove it from the
	x := dbIndexForHashesPerNodePrefix(addr)
	if k, _ := s.db.Get(x, nil); !bytes.HasPrefix(k, x) {
		batch.Delete(dbIndexForNodes(addr))
	}

	x = dbIndexForNodesWithHashPrefix(key)
	if k, _ := s.db.Get(x, nil); !bytes.HasPrefix(k, x) {
		batch.Delete(dbIndexForHashes(key))
	}
	return s.db.Write(batch, nil)
}
...
func (s *GlobalStore) keys(addr *common.Address, startKey []byte, limit int) (keys mock.Keys, err error) {
	...
	prefix := []byte{dbIndexForHashesPrefix}
	if addr != nil {
		prefix = dbIndexForHashesPerNodePrefix(*addr)
	}
	if startKey != nil {
		if addr != nil {
			startKey = dbIndexForHashesPerNode(*addr, startKey)
		} else {
			startKey = dbIndexForHashes(startKey)
		}
	} else {
		startKey = prefix
	}
...
func (s *GlobalStore) nodes(key []byte, startAddr *common.Address, limit int) (nodes mock.Nodes, err error) {
	...
	prefix := []byte{dbIndexForNodesPrefix}
	if key != nil {
		prefix = dbIndexForNodesWithHashPrefix(key)
	}
	startKey := prefix
	if startAddr != nil {
		if key != nil {
			startKey = dbIndexForNodesWithHash(key, *startAddr)
		} else {
			startKey = dbIndexForNodes(*startAddr)
		}
	}
...
func (s *GlobalStore) Export(w io.Writer) (n int, err error) {
	...
	iter := snap.NewIterator(util.BytesPrefix([]byte{dbIndexForHashesByNodePrefix}), nil)
	defer iter.Release()
	...
	for iter.Next() {
		k := bytes.TrimPrefix(iter.Key(), []byte{dbIndexForHashesByNodePrefix})
...
const (
	...
	indexDataPrefix                 = 0
	dbIndexForNodesWithHashesPrefix = 1
	dbIndexForHashesByNodePrefix    = 2
	dbIndexForNodesPrefix           = 3
	dbIndexForHashesPrefix          = 4
...
// dbIndexForHashesPerNode constructs a database key to store keys used in
// NodeKeys method.
func dbIndexForHashesPerNode(addr common.Address, key []byte) []byte {
	return append(dbIndexForHashesPerNodePrefix(addr), key...)
}

// dbIndexForHashesPerNodePrefix returns a prefix containing a node address used in
// NodeKeys method. Node address is hex encoded to be able to use keyTermByte
// for splitting node address and key.
func dbIndexForHashesPerNodePrefix(addr common.Address) []byte {
	return append([]byte{dbIndexForNodesWithHashesPrefix}, append([]byte(addr.Hex()), keyTermByte)...)
}

// dbIndexForNodesWithHash constructs a database key to store keys used in
// KeyNodes method.
func dbIndexForNodesWithHash(key []byte, addr common.Address) []byte {
	return append(dbIndexForNodesWithHashPrefix(key), addr[:]...)
}

// dbIndexForNodesWithHashPrefix returns a prefix containing a key used in
// KeyNodes method. Key is hex encoded to be able to use keyTermByte
// for splitting key and node address.
func dbIndexForNodesWithHashPrefix(key []byte) []byte {
	return append([]byte{dbIndexForHashesByNodePrefix}, append([]byte(common.Bytes2Hex(key)), keyTermByte)...)
}

// dbIndexForNodes constructs a database key to store keys used in
// Nodes method.
func dbIndexForNodes(addr common.Address) []byte {
	return append([]byte{dbIndexForNodesPrefix}, addr[:]...)
}

// dbIndexForHashes constructs a database key to store keys used in
// Keys method.
func dbIndexForHashes(key []byte) []byte {
	return append([]byte{dbIndexForHashesPrefix}, key...)
}
