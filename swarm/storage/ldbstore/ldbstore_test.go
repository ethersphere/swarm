...
func TestMarkAccessed(t *testing.T) {
	...
	h := GenerateRandomChunk(constants.DefaultChunkSize)
...
func testIterator(t *testing.T, mock bool) {
	...
	chunks := GenerateRandomChunks(constants.DefaultChunkSize, chunkcount)
...
func testLDBStoreRemoveThenCollectGarbage(t *testing.T) {
	...
	chunks := make([]Chunk, n)
	for i := 0; i < n; i++ {
		c := GenerateRandomChunk(constants.DefaultChunkSize)
		chunks[i] = c
