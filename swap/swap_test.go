...
func newBaseTestSwapWithParams(t *testing.T, key *ecdsa.PrivateKey, params *Params, backend *swapTestBackend) (*Swap, string) {
	...
	factory, err := swapcontract.FactoryAt(backend.factoryAddress, backend)
	if err != nil {
		t.Fatal(err)
	}
...
func TestVerifyContractNotDeployedByFactory(t *testing.T) {
	...
	if err = swap.chequebookFactory.VerifyContract(addr); err != swapcontract.ErrNotDeployedByFactory {
...
func TestFactoryAddressForNetwork(t *testing.T) {
	address, err := swapcontract.FactoryAddressForNetwork(3)
	if err != nil {
...
func TestFactoryVerifySelf(t *testing.T) {
	...
	factory, err := swapcontract.FactoryAt(testBackend.factoryAddress, testBackend)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err = swapcontract.FactoryAt(addr, testBackend); err != nil {
		t.Fatal(err)
	}

	if _, err = swapcontract.FactoryAt(common.Address{}, testBackend); err != nil {
		t.Fatal(err)
	}
}
