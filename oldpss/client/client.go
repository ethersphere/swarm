...
// The oldpss client provides devp2p emulation over oldpss RPC API,
// giving access to oldpss methods from a different process
type Client struct {
	BaseAddrHex string
...
type pssRPCRW struct {
	*Client
	topic    string
	msgC     chan []byte
	addr     oldpss.PssAddress
	pubKeyId string
	lastSeen time.Time
	closed   bool
}

func (c *Client) newpssRPCRW(pubkeyid string, addr oldpss.PssAddress, topicobj message.Topic) (*pssRPCRW, error) {
...
func (rw *pssRPCRW) WriteMsg(msg p2p.Msg) error {
	...
	pmsg, err := rlp.EncodeToBytes(oldpss.ProtocolMsg{
		Code:    msg.Code,
		Size:    msg.Size,
		Payload: rlpdata,
	})
...
func NewClientWithRPC(rpcclient *rpc.Client) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cannot get oldpss node baseaddress: %v", err)
	}
	return client, nil
}
...
// Mounts a new devp2p protcool on the oldpss connection
//
// the protocol is aliased as a "oldpss topic"
...
func (c *Client) RunProtocol(ctx context.Context, proto *p2p.Protocol) error {
	...
	if err != nil {
		return fmt.Errorf("oldpss handshake activation failed: %v", err)
	}
		...
		for {
			...
			case msg := <-msgC:
				...
				if c.peerPool[topicobj][pubkeyid] == nil {
					...
					addr := oldpss.PssAddress(addrbytes)
...
// Add a oldpss peer (public key) and run the protocol on it
//
// client.RunProtocol with matching topic must have been
// run prior to adding the peer, or this method will
// return an error.
//
// The key must exist in the key store of the oldpss node
...
func (c *Client) AddPssPeer(pubkeyid string, addr []byte, spec *protocols.Spec) error {
	topic := oldpss.ProtocolTopic(spec)
...
// Remove a oldpss peer
//
// TODO: underlying cleanup
func (c *Client) RemovePssPeer(pubkeyid string, spec *protocols.Spec) {
	log.Debug("closing oldpss client peer", "pubkey", pubkeyid, "protoname", spec.Name, "protoversion", spec.Version)
	c.poolMu.Lock()
