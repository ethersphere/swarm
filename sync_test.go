...
func TestSync(t *testing.T) {
	...
	for i := 1; i <= iterationCount && !errored; i++ {
		nodeIndex := 0
		...
		fmt.Println("sync simulation syncing", "iteration", i, "syncing", time.Since(startSyncing))
		...
		for ni, n := range nodes {
			...
			if uint64(si["retrievalDataIndex"]) > dbCapacity {
				errored = true
				t.Errorf("retrieval data index overflowed over db capacity. index size: %d, capacity: %d", si["retrievalDataIndex"], dbCapacity)
