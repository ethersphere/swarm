...
func startStop(net *Network, quit chan struct{}, nodeCount int) {
	nodes := connectNodesInRing(net, nodeCount)
	...
	for {
		...
		for i := lowid; i < highid; i++ {
			...
			case <-quit:
				log.Info("Terminating simulation loop")
				return
			case <-time.After(time.Duration(randWait) * time.Millisecond):
			}
...
func connectNodesInRing(net *Network, nodeCount int) []discover.NodeID {
