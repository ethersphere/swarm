...
func TestBzzHandshakeNetworkIDMismatch(t *testing.T) {
	...
	err = s.testHandshake(
		...
		&p2ptest.Disconnect{Peer: node.ID(), Error: fmt.Errorf("Message handler error: (msg code 0): network id mismatch 321 (!= %v)", TestProtocolNetworkID)},
	)
...
func TestBzzHandshakeVersionMismatch(t *testing.T) {
	...
	err = s.testHandshake(
		...
		&p2ptest.Disconnect{Peer: node.ID(), Error: fmt.Errorf("Message handler error: (msg code 0): version mismatch 0 (!= %d)", TestProtocolVersion)},
	)
...
func TestBzzHandshakeInvalidCapabilities(t *testing.T) {
	...
	err = s.testHandshake(
		correctBzzHandshake(s.addr, lightNode),
		msg,
		&p2ptest.Disconnect{Peer: node.ID(), Error: fmt.Errorf("Message handler error: (msg code 0): invalid capabilities setting: %s", msg.Addr.Capabilities)},
	)
